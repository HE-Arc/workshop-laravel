...
Ils ne sont pas utiles pour pour la paration au workshop !
...
4. Push les 2 fichiers créés : .gitmodules et laradock (le contenu de laradock ne doit PAS être poussé, seul un fichier nommé laradock doit être poussé)
    > Vos co-équipiers peuvent pull à partir d'ici et faire les mêmes étapes de leur côté. ATTENTION néanmoins, vos co-équipiers devront récupérer Laradock (voir le chapitre de ce README nommé "Récupérer le projet").
...
13. Modifier le `.env` pour correspondre à votre configuration, si toutes les étapes ont étés suivies voici ce qu'il faudra modifier.
...
1. La première choses que nous allons faire c'est d'ajouter laravel/breeze (scaffolder/starter-kit officiel pour Laravel) à notre projet à l'aide de composer.

```sh
composer require laravel/breeze --dev
```

2. Une fois laravel/breeze ajouté au projet, nous allons pouvoir l'utiliser. Dans notre cas nous souhaitons utiliser la version avec VueJS.
...
3. Si vous souhaitez utiliser tailwindcss, vous pouvez sauter les étapes concernant la conversion de tailwindcss à bootstrap et reprendre depuis l'étape (7.).

4. Afin d’utiliser Bootstrap à la place de tailwindcss, nous allons utiliser un package bien pratique. Ce package va convertir notre code tailwindcss en bootstrap.
...
5. Puis utiliser le package pour convertir le code tailwindcss en bootstrap.

```sh
php artisan jetstrap:swap breeze-inertia
```

6. Il reste encore à supprimer tailwindcss et ses éventuels plugins du projet. Pour se faire direction package.json et supprimer tous les packages contenant tailwindcss dans leur nom.
...
9. Tester que tout fonctionne en accédant à http://localhost:8000, vous devriez avoir un page Laravel avec un bouton login et register en haut à gauche. Vous pouvez tester de vous créer un compte pour vérifier que tout fonctionne correctement, si c'est le cas, vous devriez être prêt :)

10. Si vous avez des soucis n'hésitez pas à me contacter par mail ou autre.
